...
namespace Yucom.Controllers
{
    ...
    public class CuentasController : ControllerBase
    {
        ...
        [HttpPost("Registrar")]
        public async Task<ActionResult<RespuestaAutenticacion>> Registrar(CuentaRegistroDTO cuentaRegistroDTO)
        {
            var usuario = new IdentityUser {UserName = cuentaRegistroDTO.Email, 
                Email = cuentaRegistroDTO.Email};
            var existe = await UserManager.GetEmailAsync(usuario);
            var resultado = await UserManager.CreateAsync(usuario, cuentaRegistroDTO.Password);

            if (resultado.Succeeded)
            {
                return await ConstruirToken(cuentaRegistroDTO);
            }
            else
            {
                return BadRequest("Ya existe una cuenta registrada con ese usuario o correo electronico");
            }
        }
