...
namespace Yucom.Controllers
{
    ...
    public class PresentadorController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<PresentadorCreationDTO>>> Get([FromQuery] PaginacionDTO paginacionDTO)
        {
            ...
            var presentador = await queryable.OrderBy(presentador => presentador.Nombre).Paginar(paginacionDTO)
            .Where(z => z.Status == true).ToListAsync();
        ...
        [HttpGet("{id}")]
        [AllowAnonymous]
        public async Task<ActionResult<Presentador>> Get (int id)
        {
            var nombre = await context.Presentadors.FirstOrDefaultAsync(x => x.Id == id);
        ...
        [HttpPut("{rfc}")]
        //[Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)] 
        public async Task<ActionResult> Put(PresentadorCreationDTO presentadorCreationDTO, string rfc)
        {
            if (presentadorCreationDTO.RFC != rfc)
            {
                return BadRequest($"El RFC {rfc} no existe en el sistema");
            }
